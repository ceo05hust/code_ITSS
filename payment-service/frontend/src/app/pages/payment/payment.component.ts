          ...
          <div style="flex: 1; min-width: 0;">
            ...
            <div class="card" style="margin-bottom: 24px;">
              ...
              <div class="summary-row">
                ...
                <span style="font-weight: 700; letter-spacing: 0.05em;">{{ paymentRef || 'Loading...' }}</span>
              </div>
              <div class="summary-row">
            ...
            <div>
              ...
              <div class="method-row" (click)="switchToCreditCard()">
                <div class="method-row-left">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/>
                  </svg>
                  Pay Order by Credit Card
                </div>
            ...
            <div class="card" style="text-align: center;">
              ...
              <div style="display: flex; gap: 10px;">
                <button class="btn btn-primary"
                        ...
                        style="flex: 1;">
                  {{ isConfirming ? 'Checking...' : 'I have already paid' }}
                </button>
...
export class PaymentComponent implements OnInit {
  ...
  loadQR() {
    ...
    this.paymentService.generateQr({
      ...
      totalProductPriceExVAT: this.totalProductPriceExVAT,
      totalProductPriceIncVAT: this.totalProductPriceIncVAT,
      ...
      next: (res) => {
        ...
        if (res.success && res.data) {
          ...
          this.paymentRef      = res.data.paymentRef ?? '';
          this.transferContent = qr?.content ?? ('AIMS ' + this.paymentRef);
  ...
  confirmPayment() {
    ...
    this.paymentService.confirmPayment({ paymentRef: this.paymentRef }).subscribe({
      next: (res) => {
        this.isConfirming = false;
        if (res.success && res.data?.status === 'SUCCESS') {
          this.router.navigate(['/success'], { state: { transactionId: res.data.transactionId, totalAmount: this.totalAmount } });
        } else {
          this.router.navigate(['/payment-failed'], { state: { reason: res.message || 'Payment was not completed.' } });
        }
        this.cdr.detectChanges();
      },
      error: (err) => {
        this.isConfirming = false;
        const reason = err?.error?.message ?? 'An unexpected error occurred. Please try again.';
        this.router.navigate(['/payment-failed'], { state: { reason } });
        this.cdr.detectChanges();
      }
    });
  }
  ...
  switchToCreditCard() {
    alert('Credit Card payment is not available in this demo.');
  }
